...
func GetAllProgrammingSkills(c *gin.Context) {
	// Get profile ID from the request parameters
	profileID, err := strconv.ParseUint(c.Param("profileID"), 10, 64)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid ProfileID"})
	...
	// Query programming skills associated with the profile
	var programmingSkills []models.Programming
	if err := initializers.DB.Where("skill_id IN (SELECT id FROM skills WHERE profile_id = ?)", profileID).Find(&programmingSkills).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to retrieve programming skills"})
		return
	}

	c.JSON(http.StatusOK, gin.H{"programmingSkills": programmingSkills})
}